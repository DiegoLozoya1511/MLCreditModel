...
def compute_thresholds(r: pd.Series, RB: pd.Series, EAD: pd.Series, LGD: float = LGD) -> pd.Series:
    """
    ...
    Parameters:
        ...
        RB  : np.ndarray or pd.Series, shape (n_clients,) - Revolving Balance (BILL_AMT1 - PAY_AMT1) for each client.
...
def get_predictions(model, X: pd.DataFrame, r: pd.Series, RB: pd.Series, EAD: pd.Series, LGD: float = LGD) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    Parameters:
        ...
        RB: pandas Series of revolving balance for each client
    ...
    thresholds = compute_thresholds(r, RB, EAD, LGD)
...
def realized_pnl(r: pd.Series, RB: pd.Series, EAD: pd.Series, pred: np.ndarray, y: pd.Series, LGD: float = LGD) -> np.ndarray:
    """
    ...
    pnl = np.where(pred == 0, 0,            # didn't lend
                   np.where(y == 0, RB * r, # lent → no default
                            -EAD * LGD))    # lent → default

    return pnl


def benchmark_pnl(r: pd.Series, RB: pd.Series, EAD: pd.Series, y: pd.Series) -> np.ndarray:
    """
    ...
    return realized_pnl(r, RB, EAD, pred_all, y)
